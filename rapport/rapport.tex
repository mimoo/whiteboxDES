...
Ces matrices contienent généralement beaucoup trop de zéros (identités, permutations, ...). Pour éviter d'avoir trop de Lookup tables nulles et réduire le champ de recherche pour l'attaquant, chaque matrice est découpé en \textbf{blocs de sous-matrices}.\\
...
Pour 8 bits d'entrées il y a $2^{8}$ possibilités. Donc la taille de la lookuptable est de $2^{8} * sizeof(output)$.\\
Pour 16 bits d'entrées, il y a $2^{16}$ possibilités. Donc la taille de la lookup table est de $2^{16} * sizeof(output)$.\\
Avec notre exemple simplifié, en multipliant par deux le nombre de Lookup table on réduit la taille de notre fichier final par $(2^{16} - 2^8 + 2^8) * sizeof(output)$ pour chaque Lookup table.
etc...\\
