...
\subsubsection{Les 3 grandes étapes de DES}	
...
\begin{itemize}
\item Expansion du bloc de droite vers 48 bits (on copie 16 bits du bloc initial). 
\item XOR avec la clé correspondant au numéro d'itération actuelle.
\item Substitutions de 6 bits vers 4 bits (8 tables de substitutions sont utilisées). Les 4 bits du milieu déterminent la colonne de la table et les 2 bits extérieurs la ligne. Le nouveau bloc a alors une taille de 32 bits.
\item Permutation sur ce bloc.
\item XOR entre ce bloc et le bloc de gauche.
\item Les deux blocs sont échangés.
\end{itemize}

\item Fusionner les deux blocs, et effectuer une permutation finale, inverse de la permutation initiale.
\end{enumerate}
...
puisqu'on a, au maximum un état de 96bits dans la mémoire, on va garder cet état de 96 bits tout le long et utiliser les bits qui ne servent à rien comme bypass, pour cela, nous garderons :

Pour l'état 1 et l'état 3 : le 32 bits de la partie gauche, les 48 bits de la partie droite étendue et les 16 bits non étendus de la partie droite. 

Pour l'état 2 : les 32 bits de la partie droite, les 32 bits résultants du Xor entre la partie droite étendue et la clé et les 32 buts de la partie droite.
...
M2 est une matrice (96,96). Elle réalise la permutation, le xor avec la partie gauche, l'expansion pour le prochain round, met R(r) dans L(r+1) et extrait le vrai R(r).
